class Colour {
	...
	generateHex() {
		...
		let color = "#";
		for (let i = 0; i < 6; i++) {
			color += chars[Math.floor(Math.random() * 16)]; /*floor for a whole number/not decimal, random for a random number, 16 for the number of characters)*/
		}
	...
	copyToClipboard() {
		...
		input.blur(); /*so it isn't selected - like when you highlight/select text and it has a blue background*/

        /*visual feedback that it has been copied*/
